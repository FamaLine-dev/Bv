...
func BroadcastFrame(agentID string, frameData map[string]interface{}) {
    ...
    // ✅ FIXED: Validate frame has data before broadcasting
    hasData := false
    if frameData != nil {
        if _, ok := frameData["data"]; ok {
            hasData = true
        }
    }
    
    if !hasData {
        log.Printf("⚠️ [BROADCAST] Skipping broadcast - no frame data for agent %s", agentID)
        return
    }
    ...
    if len(wsClients) > 0 {
        log.Printf("📡 Broadcasting frame to %d WebSocket clients (agent: %s)", len(wsClients), agentID)
    }
