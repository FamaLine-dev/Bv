...
func (s *Server) handleScreenFrame(agent *Agent, result map[string]interface{}) {
    // ✅ FIXED: Better error handling and logging for screen frames
    if frameData, ok := result["data"].(string); ok {
        ...
        // ✅ Added: Debug logging if frame data is missing
        log.Printf("⚠️ [FRAME_WARNING] No 'data' field in screen frame from %s", agent.ID)
        log.Printf("🔍 [DEBUG] Result keys: %v", getResultKeys(result))
